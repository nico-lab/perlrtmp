...
use constant HANDSHAKE_SERVER_TO_CLIENT => 1;
use constant HANDSHAKE_COMPLETE => 2;
use constant HANDSHAKE_SIZE => 1536;

use constant HANDSHAKE_SERVER => pack('C*', 
...
use constant KEY_SERVER => pack('C*', 
...
sub new {
	...
	my $hash = {
		rtmp => $rtmp,
		phase => HANDSHAKE_CLIENT_TO_SERVER,
	};
...
sub complete {
	my($s) = @_;
	return $s->{phase} == HANDSHAKE_COMPLETE;
}

sub execute {
	my($s) = @_;

	if ($s->{phase} == HANDSHAKE_CLIENT_TO_SERVER) {
		$s->clientToServer();
	} elsif ($s->{phase} == HANDSHAKE_SERVER_TO_CLIENT) {
		$s->serverToClient();
	}
}
...
sub getHandshake {
	...
	if ($schema == 0) {
		...
		for (my $i = 0; $i < 1504; $i++) {
			push(@server_a, int(rand(256)));
		}

		my $server = pack('C*', @server_a);
